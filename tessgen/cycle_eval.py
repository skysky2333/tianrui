...
from .ckpt import EdgeBundle, NodeDiffusionBundle, SurrogateBundle
...
from .utils import Batch, ensure_dir, format_progress_bar
...
def run_cycle_eval(
    *,
    ...
    End-to-end evaluation:
      metrics + RD -> (node diffusion -> edge model) -> surrogate -> metrics

    Writes:
      - rows.jsonl (per-row details)
      - figures/ (summary plots, PNG+SVG)
      - graphs_single/, graphs_best/ (per-row graph plots, PNG+SVG) if save_row_figs
    """
    out = Path(out_dir)
    ensure_dir(str(out))
    ...
    with open(rows_path, "w") as f_rows:
        for n_done, row_idx in enumerate(row_indices, start=1):
            row_idx = int(row_idx)
            r = df.iloc[row_idx]
            ...
            cond_full = torch.cat([rd_t, cond_vals], dim=-1)
            ...
            for j in range(k_best):
                n_nodes = sample_node_count(node_bundle.n_pred, cond_z, min_n=int(min_n), max_n=int(max_n))
            ...
            if save_graph_files:
                for mode, cand in [("single", single), ("best", best)]:
                    coords = cand["coords01"] * COORD_RANGE + COORD_MIN
                    ...
                    with open(conn_path, "w") as fe:
                        for eid, (u, v) in enumerate(cand["edges_uv"].tolist(), start=1):
                            fe.write(f"{eid}, {int(u)+1}, {int(v)+1}\n")

            f_rows.write(json.dumps(row_record) + "\n")

            if int(progress_every) > 0 and (n_done % int(progress_every) == 0 or n_done == len(row_indices)):
                msg = format_progress_bar(done=n_done, total=len(row_indices), start_time=t0, prefix="cycle")
                end = "\n" if n_done == len(row_indices) else "\r"
