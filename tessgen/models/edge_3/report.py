...
def eval_edge3_model(
    *,
    ...
    for sample in dl:
        ...
        cand = candidate_pairs(coords01_cpu.numpy(), cand_mode=str(lit.cand_mode), k=int(lit.k))
        if cand.shape[0] == 0:
            continue
...
def make_report_and_figures(
    *,
    ...
    if any("val/surrogate_rs_r" in r for r in hist):
        ...
        save_line_plot(
            out_path=str(figs / "surrogate_rs_r.png"),
            x=epochs,
            ys={"val/surrogate_rs_r": rs_r},
            title="Surrogate RS Pearson r (edge-sampled graphs)",
            xlabel="epoch",
            ylabel="Pearson r",
        )
